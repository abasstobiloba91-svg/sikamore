...
{/* NUCLEAR MOBILE TOUCH FIX */}
{inlineAddId !== product.id && (
  <div className="absolute inset-x-0 bottom-6 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center z- pointer-events-none">
    <div className="flex bg-white shadow-xl pointer-events-auto h-12 w-32 rounded-sm overflow-hidden border border-zinc-100 relative z- isolate">
      ...
      <div 
        ...
        onPointerDownCapture={(e) => { 
          ...
          if (!product.is_sold_out) {
            setInlineAddId(product.id); 
          }
        }} 
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}
        className={`w-1/2 flex items-center justify-center hover:bg-zinc-50 transition-colors border-r border-zinc-200 text-black cursor-pointer ${product.is_sold_out ? 'opacity-30 pointer-events-none' : ''}`}
      >
      ...
      <div 
        ...
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}
        className="w-1/2 flex items-center justify-center hover:bg-zinc-50 transition-colors text-black cursor-pointer"
      >
