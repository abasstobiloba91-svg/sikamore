...
{inlineAddId !== product.id && (
  <div className="absolute inset-x-0 bottom-4 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center z- pointer-events-none">
    {/* Much smaller width and height (w-24 h-9) with a slight backdrop blur */}
    <div className="flex bg-white/95 backdrop-blur-sm shadow-md pointer-events-auto h-9 w-24 rounded-sm overflow-hidden border border-zinc-100 relative z- isolate">
      
      {/* BAG ACTION */}
      <div 
        role="button"
        tabIndex={0}
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); if(!product.is_sold_out) setInlineAddId(product.id); }}
        onTouchEnd={(e) => { e.preventDefault(); e.stopPropagation(); if(!product.is_sold_out) setInlineAddId(product.id); }}
        className={`w-1/2 flex items-center justify-center hover:bg-zinc-50 active:bg-zinc-100 transition-colors border-r border-zinc-200 text-black cursor-pointer ${product.is_sold_out ? 'opacity-30 pointer-events-none' : ''}`}
      >
      ...
      <div 
        ...
        onTouchEnd={(e) => { e.preventDefault(); e.stopPropagation(); setIsSearchOpen(false); setSearchQuery(''); openQuickView(product); }}
        className="w-1/2 flex items-center justify-center hover:bg-zinc-50 active:bg-zinc-100 transition-colors text-black cursor-pointer"
      >
