...
export default function ShopCatalog() {
  ...
  // ALL ORIGINAL APP STATES RESTORED + UNREAD SUPPORT DOT
  const { 
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F4] text-black font-sans antialiased text-[11px] relative overflow-x-hidden pb-24">
      ...
      {/* GLOBAL ANNOUNCEMENT TICKER - MOBILE TEXT WRAP AND ALIGNMENT BUG FIXED */}
      ...
      {/* HEADER - STRUCTURAL ALIGNMENT RESTORED VIA EXPLICIT 3-COLUMN SYSTEM */}
      <header className="bg-[#F5F5F4] text-black border-b border-zinc-300 sticky top-0 z-[50] shadow-sm">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-8 h-20 sm:h-24 grid grid-cols-3 items-center">
          ...
          <div className="flex items-center justify-start">
            <Link href="/admin" className="tracking-[0.2em] text-zinc-500 hover:text-black uppercase text-[9px] sm:text-[10px]">Portal</Link>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 sm:gap-6">
            ...
            {/* BAG NOTIFICATION COMPONENT AS DISCUSSED */}
            <button onClick={() => setIsCartOpen(true)} className="relative hover:text-zinc-500 transition-colors p-1 flex items-center">
              ...
              {cartItemCount > 0 && (
                <span className="absolute -top-1 -right-1.5 w-4 h-4 bg-red-600 text-white flex items-center justify-center rounded-full text-[8px] font-bold shadow-sm">
                  {cartItemCount}
                </span>
              )}
            </button>
      ...
      {/* FILTER BAR - SORT BY LATEST COMPACT MICRO-TYPOGRAPHY APPLIED */}
      <section className="bg-[#F5F5F4] border-b border-zinc-300 sticky top-[121px] sm:top-[137px] z-30 shadow-sm">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-8 py-3.5 flex items-center justify-between">
          ...
          {/* CRITICAL MOBILE SIZE FIXED HERE TO COMPLEMENT THE GRID AS REVERSED IN image.png */}
              ...
              return (
                <div key={product.id} className="group flex flex-col relative bg-[#0A0A0A] p-1.5 shadow-xl rounded-sm">
                  <div className="bg-[#111] aspect-[3/4] w-full overflow-hidden relative flex items-center justify-center rounded-sm cursor-pointer" onClick={() => !product.is_sold_out && openQuickView(product)}>
                    ...
                    {/* INLINE QUICK ADD OVERLAY PANEL */}
                    ...
                    {/* BUTTON WIDTH AND PADDING ADJUSTED FOR FLAT SLEEK MOBILE HORIZONTAL LAYOUT AS DEMANDED */}
                    {inlineAddId !== product.id && (
                      <>
                        <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center z-10 shadow-2xl">
                          <button 
                            onClick={(e) => { e.stopPropagation(); setInlineAddId(product.id); }} 
                            disabled={product.is_sold_out} 
                            className="w-[96px] sm:w-[125px] bg-black/95 text-white py-2.5 px-1 border border-zinc-800 hover:bg-white hover:text-black transition-all duration-300 text-center tracking-[0.18em] text-[8px] sm:text-[9px] uppercase font-medium whitespace-nowrap rounded-sm"
                          >
                            Add To Bag
                          </button>
                        </div>
      ...
      <div className={`fixed inset-0 z-[130] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 transition-opacity duration-500 ${showNewsletter ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="bg-[#0A0A0A] text-white w-full max-w-md p-10 border border-zinc-800 text-center relative shadow-2xl">
          <button onClick={() => {setShowNewsletter(false); sessionStorage.setItem('sikamore_newsletter', 'true');}} className="absolute top-4 right-4 text-zinc-500 hover:text-white transition-colors">✕</button>
