...
export default function ShopCatalog() {
  ...
  // --- ANALYTICS TRANSMITTER: PRODUCT CLICK LOG ---
  const openQuickView = async (product) => {
    ...
    // Log the click event to your live analytics dashboard
    try {
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans antialiased text-[11px] relative overflow-x-hidden pb-24">
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-8 h-20 sm:h-24 grid grid-cols-3 items-center">
          ...
          <div className="flex items-center justify-end gap-3 sm:gap-6">
            ...
            <button onClick={() => setIsCartOpen(true)} className="relative hover:text-zinc-500 transition-colors p-1 flex items-center">
              ...
              {cartItemCount > 0 && (
                <span className="absolute -top-1 -right-1.5 w-3.5 h-3.5 bg-red-600 text-white flex items-center justify-center rounded-full text-[7.5px] font-bold">
                  {cartItemCount}
                </span>
              )}
            </button>
      ...
      {isMenuOpen && <div className="fixed inset-0 bg-black/40 z- backdrop-blur-sm" onClick={() => setIsMenuOpen(false)}></div>}
      ...
      <section className="bg-white border-b border-zinc-200 sticky top-[121px] sm:top-[137px] z-30">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-8 py-3.5 flex items-center justify-between">
          <button className="flex items-center gap-2 border border-zinc-200 px-3.5 py-1.5 text-[9px] uppercase tracking-wider hover:border-black hover:bg-black hover:text-white transition-colors">
            Filter
          </button>

          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 md:hidden">
              <button onClick={() => setIsListView(true)} className={`p-1.5 border transition-all ${isListView ? 'border-black bg-black text-white' : 'border-zinc-200 text-zinc-500'}`}>
              ...
              <button onClick={() => setIsListView(false)} className={`p-1.5 border transition-all ${!isListView ? 'border-black bg-black text-white' : 'border-zinc-200 text-zinc-500'}`}>
            ...
            <div className="hidden md:flex items-center gap-2">
              <button onClick={() => { setViewCols(2); setIsListView(false); }} className={`flex gap-[3px] p-2 border transition-all ${viewCols === 2 && !isListView ? 'border-black bg-black text-white' : 'border-zinc-200 text-zinc-400'}`}>
              ...
              <button onClick={() => { setViewCols(3); setIsListView(false); }} className={`flex gap-[3px] p-2 border transition-all ${viewCols === 3 && !isListView ? 'border-black bg-black text-white' : 'border-zinc-200 text-zinc-400'}`}>
              ...
              <button onClick={() => { setViewCols(4); setIsListView(false); }} className={`flex gap-[2px] p-2 border transition-all ${viewCols === 4 && !isListView ? 'border-black bg-black text-white' : 'border-zinc-200 text-zinc-400'}`}>
              ...
              return (
                <div key={product.id} className="group flex flex-col relative bg-white p-1">
                  <div className="bg-zinc-50 aspect-[3/4] w-full overflow-hidden relative flex items-center justify-center rounded-sm cursor-pointer border border-zinc-100" onClick={() => !product.is_sold_out && openQuickView(product)}>
                    <img src={product.image} alt={product.name} className="w-full h-full object-cover transition-transform duration-[1000ms] group-hover:scale-102" />
                    ...
                    <div className={`absolute inset-0 bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center p-4 z-20 text-black transition-opacity duration-300 ${inlineAddId === product.id ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                      ...
                      <button onClick={(e) => handleInlineAdd(e, product)} className="w-full bg-black text-white py-2.5 text-[8px] sm:text-[9px] tracking-widest uppercase hover:bg-zinc-800 font-medium transition-colors">
                        Confirm & Add
                      </button>
                    </div>
                      ...
                      <div className="absolute inset-0 bg-black/[0.02] opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center z-10">
                        <div className="flex gap-2 bg-white p-1.5 rounded-sm border border-zinc-100">
                          <button 
                            ...
                            className="p-2 text-black hover:text-zinc-500 transition-colors"
                          >
                          ...
                          <button 
                            ...
                            className="p-2 text-black hover:text-zinc-500 transition-colors border-l border-zinc-200"
                          >
      ...
      {/* DETAILED PRODUCT OVERLAY MODAL (NOW WITH DYNAMIC ACCORDION DATABASE MAPPING) */}
      {quickViewProduct && (
        <div className="fixed inset-0 z- flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white text-black w-full max-w-3xl flex flex-col relative border border-zinc-200 my-8 rounded-sm">
            <button onClick={() => setQuickViewProduct(null)} className="absolute top-4 right-4 z-10 text-zinc-400 hover:text-black transition-colors bg-white/80 p-1.5 rounded-full border border-zinc-100">
      ...
      <div className={`fixed inset-0 z- flex items-center justify-center bg-black/60 backdrop-blur-md p-4 transition-opacity duration-500 ${showNewsletter ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="bg-[#0A0A0A] text-white w-full max-w-md border border-zinc-900 relative flex flex-col max-h-[90vh] overflow-hidden rounded-sm shadow-2xl">
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans antialiased text-[11px] relative overflow-x-hidden pb-24">
      ...
      {/* DYNAMIC TWO-STAGE INTERACTIVE CONVERSION POPUP MODAL */}
      <div className={`fixed inset-0 z-[150] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 transition-opacity duration-500 ${showNewsletter ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="bg-[#0A0A0A] text-white w-full max-w-md border border-zinc-900 relative flex flex-col max-h-[90vh] overflow-hidden rounded-sm shadow-2xl">
          <button onClick={() => { setShowNewsletter(false); sessionStorage.setItem('sikamore_newsletter', 'true'); }} className="absolute top-4 right-4 text-zinc-500 hover:text-white z-50 bg-[#111] p-1.5 rounded-full border border-zinc-800">
            ✕
          </button>
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans antialiased text-[11px] relative overflow-x-hidden pb-24">
      ...
      {/* PURE PLAIN NAVIGATION HEADER SYSTEM (image_8.png) */}
      <header className="bg-white text-black border-b border-zinc-200 sticky top-0 z-[50]">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-8 h-20 sm:h-24 grid grid-cols-3 items-center">
      ...
      {/* PLAIN SIDE NAVIGATION DRAWER INDEX (image_9.png) */}
      <div className={`fixed inset-y-0 left-0 z-[140] w-[280px] bg-white text-black shadow-2xl transform transition-transform duration-500 ease-in-out ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
        <div className="p-6 border-b border-zinc-200 flex justify-between items-center">
          <span className="text-[10px] tracking-[0.3em] font-serif uppercase">Index Menu</span>
          <button onClick={() => setIsMenuOpen(false)} className="text-zinc-400 hover:text-black transition-colors p-1">
      ...
      {/* PURE PLAIN PRODUCT GRID - REMOVED BLACK BOX FRAMES & SHADOWS (image_8.png) */}
      <main className="max-w-[1600px] mx-auto px-4 sm:px-8 py-6 sm:py-16 bg-white">
        {loading ? (
          <div className="text-center py-32 tracking-[0.3em] text-zinc-500 uppercase text-[9px]">Loading Archive...</div>
        ) : (
          <div className={`grid ${isListView ? 'grid-cols-1 gap-y-6 max-w-xl mx-auto' : `grid-cols-2 ${viewCols === 2 ? 'md:grid-cols-2' : viewCols === 3 ? 'md:grid-cols-3' : 'md:grid-cols-3 lg:grid-cols-4'} gap-x-4 sm:gap-x-6 gap-y-8 sm:gap-y-12`}`}>
            {products.map((product) => {
              ...
              return (
                <div key={product.id} className="group flex flex-col relative bg-white p-1">
                  {/* Image Card Grid with Plain Actions */}
                  <div className="bg-zinc-50 aspect-[3/4] w-full overflow-hidden relative flex items-center justify-center rounded-sm cursor-pointer border border-zinc-100" onClick={() => !product.is_sold_out && openQuickView(product)}>
                    <img src={product.image} alt={product.name} className="w-full h-full object-cover transition-transform duration-[1000ms] group-hover:scale-102" />
      ...
      {quickViewProduct && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white text-black w-full max-w-3xl flex flex-col relative border border-zinc-200 my-8 rounded-sm">
            ...
            {/* EXPANDABLE ACCORDION ELEMENT MODULE */}
            <div className="border-t border-zinc-100 bg-white p-6 sm:p-10 space-y-2">
              <div className="border border-zinc-200 rounded-sm overflow-hidden">
                <button onClick={() => setOpenAccordion(openAccordion === 'description' ? '' : 'description')} className="w-full bg-zinc-50 px-4 py-3 flex justify-between items-center text-[10px] tracking-widest uppercase text-zinc-700 font-medium">
                  <span>Description</span>
                ...
                {openAccordion === 'description' && (
                  <div className="p-4 text-zinc-500 text-[10px] leading-relaxed uppercase tracking-wide bg-white border-t border-zinc-100">
              ...
              <div className="border border-zinc-200 rounded-sm overflow-hidden">
                ...
                {openAccordion === 'additional' && (
                  <div className="p-4 text-zinc-500 text-[10px] leading-relaxed uppercase tracking-wide bg-white border-t border-zinc-100 space-y-1">
      ...
      {/* SLIDING MINI BAG DRAWER CONTAINER */}
      <div className={`fixed inset-y-0 right-0 z-[110] w-full sm:w-[400px] bg-[#0A0A0A] text-white shadow-2xl transform transition-transform duration-500 ease-in-out ${isCartOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
        <div className="flex items-center justify-between p-6 border-b border-zinc-900 shrink-0">
          <h2 className="text-[11px] tracking-[0.2em] uppercase font-medium">Shopping Cart ({cartItemCount})</h2>
          <button onClick={() => setIsCartOpen(false)} className="text-zinc-500 hover:text-white transition-colors">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
      ...
      <footer className="border-t border-zinc-200 bg-white pt-16 pb-12 mt-16 sm:mt-20 text-black relative z-40">
        ...
        {/* Logo block signature placement matching image_6.png */}
        <div className="max-w-[1600px] mx-auto px-4 sm:px-8 mb-12 text-center border-b border-zinc-100 pb-12">
          <h2 className="text-xl sm:text-3xl tracking-[0.5em] uppercase font-normal text-black pl-[0.5em] select-none font-serif font-bold">
            S. SIKAMÒRE
          </h2>
