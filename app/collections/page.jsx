...
export default function CollectionsPage() {
  ...
  // State to hold the latest images for each category
  ...
  // Global ticker to cycle through images in motion
  ...
  // 2. Fetch the latest products per category to use as motion backgrounds
  useEffect(() => {
    async function fetchCategoryImages() {
      ...
      if (data) {
        ...
        data.forEach(product => {
          ...
          const img = getPrimaryImage(product.image);
          if (img && categorized[cat] && categorized[cat].length < 5) {
            categorized[cat].push(img);
          }
        });

        setCategoryImages(categorized);
      }
    }
    fetchCategoryImages();
  }, []);

  // 3. Start the motion timer (Changes image every 4 seconds)
  useEffect(() => {
  ...
  // Reusable Component for each full-screen category slide
  const CategorySlide = ({ title, categoryName, images }) => {
    const currentImgIndex = images.length > 0 ? tick % images.length : 0;

    return (
      <div className="h-screen w-full snap-start relative flex flex-col items-center justify-center overflow-hidden bg-[#0A0A0A]">
        {/* Motion Background */}
        ...
        {/* Swipe Down Indicator (Only shows if there's more to see) */}
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center opacity-50 animate-bounce">
          <span className="text-[7px] tracking-[0.3em] text-white uppercase mb-2">Swipe</span>
          <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" d="M19 14l-7 7m0 0l-7-7m7 7V3" /></svg>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="h-screen w-full bg-[#0A0A0A] font-sans antialiased text-white overflow-hidden relative">
      ...
      {/* SWIPEABLE CONTAINER */}
      <div className="h-screen w-full overflow-y-auto snap-y snap-mandatory scrollbar-none relative z-10" style={{ scrollBehavior: 'smooth' }}>
        
        {/* Category 1 */}
        <CategorySlide title="Bags" categoryName="bags" images={categoryImages.bags} />
        
        {/* Category 2 */}
        <CategorySlide title="Accessories" categoryName="accessories" images={categoryImages.accessories} />
        
        {/* Category 3 */}
        <CategorySlide title="Clothing" categoryName="clothing" images={categoryImages.clothing} />

        {/* LUXURY FOOTER SNAP */}
        <div className="snap-start w-full bg-[#0A0A0A] flex flex-col items-center justify-center py-24 sm:py-32 px-6">
